...
""
import time
wb = load_workbook(r"/GMB/product/Data/GMB Product URL.xlsx")
ws = wb.active
        ...
        for r1 in driver.find_elements(By.CLASS_NAME,'zTXfQc'):
            for r2 in r1.find_elements(By.CLASS_NAME,'TB4jmc'):
                for r3 in r2.find_elements(By.CLASS_NAME,"bgmvLc"):
                    print(r3.text)
                    if r3.text == "Samsung Galaxy S22 Series":
                        r2.click()
                        time.sleep(3)
                        try:
                            driver.find_element(By.ID, "c31").clear()
                            driver.find_element(By.ID, "c31").send_keys("https://bit.ly/Samsung_Galaxy_S22_series")
                            time.sleep(3)
                            time.sleep(3)
                            ...
                            driver.find_element(By.ID, "c65").clear()
                            driver.find_element(By.ID, "c65").send_keys("https://bit.ly/Samsung_Galaxy_S22_series")
                            time.sleep(3)
